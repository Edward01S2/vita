...
function add_to_admin_body_class($classes) { 
    ...
    if($post && 'upload' !== $current_screen->base && 'list' !== $media_library_mode) { 
...
$cyrillic = of_get_option('of_cyrillic_chars');
  ...
  if ($cyrillic == 'Yes') $cyrillic_chars = ':cyrillic,latin,latin-ext';
  
  $standard_fonts = array("Arial","Georgia","Tahoma","Verdana","Helvetica");
          ...
          WebFontConfig = {
            google: { families: [ ";
				...
				foreach ($option_fonts as $font) {
					//make sure it's not a standard web font
					if (!in_array($font['face'], $standard_fonts)) {
						$italic = $font['style'];
