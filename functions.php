...
/**
 * Class to add additional options to image gallery popup
 */

if ( ! class_exists( 'Custom_Gallery_Setting' ) ) :
...
if (!function_exists('themewich_render_post_columns')) :
    function themewich_render_post_columns($column_name, $id) {
        ...
        case 'featured':
            ...
            if ($featured === "Yes") :
                echo '<input type="checkbox" id="featured-' . $id . '" name="featured-' . $id . '" checked="checked" disabled/>';
            else :
                echo '<input type="checkbox" id="featured-' . $id . '" name="featured-' . $id . '" disabled/>';
            endif;
            break;
        case 'medium':
            $medium = get_the_terms($id, 'filter');
            $terms_string = join(', ', wp_list_pluck($medium, 'name'));
            echo $terms_string;
            break;
        }
    }
...
if (!class_exists('Themewich_Walker_Portfolio_Filter')) :
    class Themewich_Walker_Portfolio_Filter extends Walker_Category {
       ...
       function start_el(&$output, $category, $depth = 0, $args = array(), $current_object_id = 0) {
          ...
          if(self::$count == 0) {
            $link = '<a href="#" class="active no-ajaxy" data-filter=".'.strtolower(preg_replace('/\s+/', '-', $cat_slug)).'" ';
          }
          else 
    ...
    function themewich_register_required_plugins() {
        ...
        $plugins = array(
            ...
            array(
                ...
                'source'                => get_template_directory() . '/functions/plugins/themewich-shortcodes.zip', // The plugin source
            ...
            array(
                'name'                  => 'Sidekick (Help Walkthroughs)', // The plugin name
 ...
 * Sidekick Constants
 */
define( 'SK_PRODUCT_ID' , 456 );
define( 'SK_ENVATO_PARTNER', '8pBf3fklQdrXKjvtK9JmyggRWxmAhae6AY4iGNqc3Ag=' );
